...
set -x 
touch /tmp/Automation/current.txt
...
#Fetch Java Version
java_version="`cat /opt/netiq/common/jre/release  |grep JAVA_VERSION= | cut -d '"' -f2`"
...
column -t -s',' -N Driver,Version,Result data >current.txt
...
printf "\n\n" >> driver.html
#sort -k1 -n data|column -t -s',' -N Driver,Version,Result >> current.txt
...
#sort -k1 -n data |column -t -s',' -N 'Third Party jar',Version,Result>> current.txt
...
echo "Engine $total $pass $fail $unknown" > bvt_result