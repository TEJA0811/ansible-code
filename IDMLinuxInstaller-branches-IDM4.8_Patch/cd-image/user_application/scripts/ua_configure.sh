...
modify_nginx_conf()
{
	...
	then
		grep "server_name  localhost" /opt/netiq/common/nginx/nginx.conf &> /dev/null
		if [ $? -ne 0 ]
		then
			export nginxconfcustomnotneeded=true
		fi
	fi
	if [ -f /opt/netiq/common/nginx/nginx.conf ] && [ -z "$nginxconfcustomnotneeded" ]
	then
	...
	if [ -f /opt/netiq/common/nginx/nginx.conf ]
	then
		#new conditions from 481
		grep "[[:blank:]]*load_module[[:blank:]]*modules/ngx_http_headers_more_filter_module.so" /opt/netiq/common/nginx/nginx.conf &> /dev/null
		if [ $? -ne 0 ]
		then
			sed -i.bak "s#worker_processes#load_module modules/ngx_http_headers_more_filter_module.so;\nworker_processes#g" /opt/netiq/common/nginx/nginx.conf
		fi
		grep "[[:blank:]]*more_clear_headers[[:blank:]]*Server" /tmp/1.txt &> /dev/null
		if [ $? -ne 0 ]
		then
			sed -i.bak "s#default_type#more_clear_headers Server;\n    default_type#g" /opt/netiq/common/nginx/nginx.conf
		fi
	fi
...
modify_context_xml()
{
   str1=`gettext install "Copying Tomcat context.xml"`
   write_and_log "$str1"
...
XMLOUT`
  ...
  then
    if [ "${UA_WFE_DB_PLATFORM_OPTION}" == "postgres" ]
    then
    	echo "Just a placeholder" &> /dev/null
    elif [ "${UA_WFE_DB_PLATFORM_OPTION}" == "oracle" ]
    then
    	echo "Just a placeholder" &> /dev/null
    elif [ "${UA_WFE_DB_PLATFORM_OPTION}" == "mssql" ]
...
if [ ! -z "$temporaryfileback" ] && [ "$temporaryfileback" == 'y' ]
then
	[ -f $IDM_TEMP/configupdate.properties ] && cp $IDM_TEMP/configupdate.properties /tmp/configupdate.properties.UA.JustbeforeCU
...
if [ ! -z "$temporaryfileback" ] && [ "$temporaryfileback" == 'y' ]
then
	[ -f $IDM_TEMP/configupdate.properties ] && cp $IDM_TEMP/configupdate.properties /tmp/configupdate.properties.UA
...
# http://localhost:8180/IDMRPT/oauth.html is introduced by configupdate code
# Fixing it with installer for now
# Should be fixed by Configupdate later by then we can remove the following line
sed -i "/com.netiq.rpt.redirect.url = http:\/\/localhost:8180\/IDMRPT\/oauth.html/d" ${ISM_CONFIG}
...
UAonlyprops-fresh()
{
   if [ $IS_UPGRADE -eq 1 ]
   then
      grep -q com.netiq.rbpmrest.clientID "${IDM_TOMCAT_HOME}/conf/ism-configuration.properties"
      [ $? -ne 0 ] && echo "com.netiq.rbpmrest.clientID = rbpmrest" >> "${IDM_TOMCAT_HOME}/conf/ism-configuration.properties"
      grep -q com.netiq.rbpmrest.response-types "${IDM_TOMCAT_HOME}/conf/ism-configuration.properties"
      [ $? -ne 0 ] && echo "com.netiq.rbpmrest.response-types = password" >> "${IDM_TOMCAT_HOME}/conf/ism-configuration.properties"
      grep -q com.netiq.idmengine.clientID "${IDM_TOMCAT_HOME}/conf/ism-configuration.properties"
      [ $? -ne 0 ] && echo "com.netiq.idmengine.clientID = idmengine" >> "${IDM_TOMCAT_HOME}/conf/ism-configuration.properties"
      grep -q com.netiq.idmengine.response-types "${IDM_TOMCAT_HOME}/conf/ism-configuration.properties"
      [ $? -ne 0 ] && echo "com.netiq.idmengine.response-types = password" >> "${IDM_TOMCAT_HOME}/conf/ism-configuration.properties"
      grep -q com.microfocus.workflow.response-types "${IDM_TOMCAT_HOME}/conf/ism-configuration.properties"
      [ $? -ne 0 ] && echo "com.microfocus.workflow.response-types = client_credentials" >> "${IDM_TOMCAT_HOME}/conf/ism-configuration.properties"
      grep -q com.netiq.rbpmrest.clientPass._attr_obscurity "${IDM_TOMCAT_HOME}/conf/ism-configuration.properties"
      [ $? -ne 0 ] && echo "com.netiq.rbpmrest.clientPass._attr_obscurity = ENCRYPT" >> "${IDM_TOMCAT_HOME}/conf/ism-configuration.properties"
   fi
}
...
{ 
    ...
    cd $UA_CONFIG_HOME >> "${LOG_FILE_NAME}"
    if [ -f ${IDM_INSTALL_HOME}user_application/IDM_Tools/WorkflowMigration.zip ]
    then
    ...
    then
     ...
     # WFE
      ...
      if [ $IS_UPGRADE -eq 1 ] && [ -f ${IDM_INSTALL_HOME}user_application/IDM_Tools/WorkflowMigration.zip ]
      then
