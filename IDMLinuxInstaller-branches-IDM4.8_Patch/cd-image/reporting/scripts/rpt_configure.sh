...
modify_server_xml()
{
    str1=`gettext install "Copying Tomcat server.xml"`
    write_and_log "$str1"
    ...
    then
        ...
        then
	       result=`"${XML_MOD}" "${IDM_TOMCAT_HOME}/conf/server.xml" "/Server/GlobalNamingResources[1]" "/" << XMLOUT 
            <Resource auth="Container" driverClassName="org.postgresql.Driver" factory="com.netiq.tomcat.jdbc.pool.CustomBasicDataSourceFactory" initialSize="10" maxIdle="10" maxTotal="50" maxWaitMillis="30000" minIdle="10" name="shared/IDMRPTCfgUpdateSource" password="${PWD}" testOnBorrow="true" type="javax.sql.DataSource" url="jdbc:postgresql://${RPT_DATABASE_HOST}:${RPT_DATABASE_PORT}/${RPT_DATABASE_NAME}${QUESTIONMARK}${AZUREPGSSL}" username="idm_rpt_cfg" validationInterval="120000" validationQuery="SELECT 1"/>
XMLOUT`
            write_log "XML_MOD Response : ${result}"

            result=`"${XML_MOD}" "${IDM_TOMCAT_HOME}/conf/server.xml" "/Server/GlobalNamingResources[1]" "/" << XMLOUT 
            <Resource auth="Container" driverClassName="org.postgresql.Driver" factory="com.netiq.tomcat.jdbc.pool.CustomBasicDataSourceFactory" initialSize="10" maxIdle="10" maxTotal="50" maxWaitMillis="30000" minIdle="10" name="shared/IDMDCSDataSource" password="${PWD}" testOnBorrow="true" type="javax.sql.DataSource" url="jdbc:postgresql://${RPT_DATABASE_HOST}:${RPT_DATABASE_PORT}/${RPT_DATABASE_NAME}${QUESTIONMARK}${AZUREPGSSL}" username="idm_rpt_data" validationInterval="120000" validationQuery="SELECT 1"/>
XMLOUT`
            write_log "XML_MOD Response : ${result}"
    
            result=`"${XML_MOD}" "${IDM_TOMCAT_HOME}/conf/server.xml" "/Server/GlobalNamingResources[1]" "/" << XMLOUT 
            <Resource auth="Container" driverClassName="org.postgresql.Driver" factory="com.netiq.tomcat.jdbc.pool.CustomBasicDataSourceFactory" initialSize="10" maxIdle="10" maxTotal="50" maxWaitMillis="30000" minIdle="10" name="shared/IDMRPTDataSource" password="${PWD}" testOnBorrow="true" type="javax.sql.DataSource" url="jdbc:postgresql://${RPT_DATABASE_HOST}:${RPT_DATABASE_PORT}/${RPT_DATABASE_NAME}${QUESTIONMARK}${AZUREPGSSL}" username="idmrptuser" validationInterval="120000" validationQuery="SELECT 1"/>
XMLOUT`
            write_log "XML_MOD Response : ${result}"
        elif [ "${RPT_DATABASE_PLATFORM_OPTION}" == "oracle" ]
	then
...
update_config_update()
{
	...
	then
	  ...
	  then
	    echo "app_versions=\"ua#4.8.0,rpt#6.6.0\"" >> $prop_file
	  else
	    echo "app_versions=\"rpt#6.6.0\"" >> $prop_file
	  fi
	else
	  rpm -qi netiq-userapp &> /dev/null
	  if [ $? -ne 0 ]
	  then
	    # userapp not installed
	    sed -i '/app_versions/d' $prop_file
	    echo "app_versions=\"rpt#6.6.0\"" >> $prop_file
	  fi
	fi
	configupdate_idm
