...
merge_ism_props()
{
    ...
    write_and_log "MERGING ismconfiguration.properties. SOURCE = ${source}, TARGET = ${target}"
    echo "" > ${TMP_FILE}
    blank=`tail -1 ${source}`
    if [ ! -z "${blank}" ]
    then 
    ...
    sed -i '/java.protocol.handler.pkgs = /d' ${source}
    sed -i '/java.protocol.handler.pkgs = /d' ${target}
    echo "java.protocol.handler.pkgs = \|com.sssw.fw.resource.net" >> ${target}
    while read line
	do
        ...
        then
            ...
            then
                ...
                sed -i "s~${match}~${line}~g" ${target} &> /dev/null
            else
