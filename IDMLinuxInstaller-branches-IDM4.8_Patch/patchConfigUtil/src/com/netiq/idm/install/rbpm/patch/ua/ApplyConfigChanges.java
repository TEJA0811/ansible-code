/*
 * ========================================================================
 *
 * Copyright (c) 2017 Unpublished Work of NetIQ Corporation. All Rights Reserved.
 *
 * THIS WORK IS AN UNPUBLISHED WORK AND CONTAINS CONFIDENTIAL,
 * PROPRIETARY AND TRADE SECRET INFORMATION OF NETIQ. ACCESS TO
 * THIS WORK IS RESTRICTED TO (I) NETIQ EMPLOYEES WHO HAVE A NEED
 * TO KNOW HOW TO PERFORM TASKS WITHIN THE SCOPE OF THEIR ASSIGNMENTS AND
 * (II) ENTITIES OTHER THAN NETIQ WHO HAVE ENTERED INTO
 * APPROPRIATE LICENSE AGREEMENTS. NO PART OF THIS WORK MAY BE USED,
 * PRACTICED, PERFORMED, COPIED, DISTRIBUTED, REVISED, MODIFIED,
 * TRANSLATED, ABRIDGED, CONDENSED, EXPANDED, COLLECTED, COMPILED,
 * LINKED, RECAST, TRANSFORMED OR ADAPTED WITHOUT THE PRIOR WRITTEN
 * CONSENT OF NETIQ. ANY USE OR EXPLOITATION OF THIS WORK WITHOUT
 * AUTHORIZATION COULD SUBJECT THE PERPETRATOR TO CRIMINAL AND CIVIL
 * LIABILITY.
 *
 * ========================================================================
 */
package com.netiq.idm.install.rbpm.patch.ua;

import java.io.File;
import java.util.List;

import org.apache.commons.configuration.XMLConfiguration;
import org.apache.commons.configuration.tree.ConfigurationNode;
import org.apache.commons.io.FileUtils;

import com.netiq.idm.install.rbpm.patch.conf.HibernateCfg;
import com.netiq.idm.install.rbpm.patch.conf.IdmUserAppLogging;
import com.netiq.idm.install.rbpm.patch.conf.IsmConfiguration;


public class ApplyConfigChanges {
    
    private static final int SUCCESS = 0;
    private static final int ERROR = 1;

    static String configChangesTemplatePath = null;
    static XMLConfiguration configChangesTemplate = null;
    
    static File configBackupDir = null;
    
    static String tomcatConfPath = null;
    
    static IsmConfiguration ismConfig = null;
    static IdmUserAppLogging idmUserAppLoggingXML = null;
    static HibernateCfg hibernateCfg = null;
    
    
    public static void main(String[] args){
        
        if(args.length < 3){
            System.out.println("Usage: com.netiq.idm.install.rbpm.patch.ua.ApplyConfigChanges "
                                    + "<Config Changes Template Path> "
                                    + "<Tomcat Home Directory> "
                                    + "<Config Backup Folder>");
            System.exit(ERROR);
        }
        
        System.out.println("Applying configuration changes...");
        
        configChangesTemplatePath = args[0];   
        tomcatConfPath =            args[1] + File.separator + "conf";              
        
        configBackupDir = new File(args[2] + File.separator + "conf");
        
        try {
            
            configChangesTemplate = new XMLConfiguration(configChangesTemplatePath);
            
           
            if(configChangesTemplate.getRootNode().getChildren("ism-configuration").size() > 0){
                updateIsmConfiguration(); 
            }
            
            if(configChangesTemplate.getRootNode().getChildren("idmuserapp_logging").size() > 0){
                updateIdmUserAppLoggingXML(); 
            }
            
            if(configChangesTemplate.getRootNode().getChildren("hibernate_cfg").size() > 0){
                updateHibernate_cfgXML(); 
            }
            
            
        } catch (Exception e) {
            
            System.err.println("There was a problem committing configuration update \n" + e.getMessage() + "\n");
            System.exit(ERROR);
            
        }
        
        System.exit(SUCCESS);
    }
    
    
    
    public static void updateIsmConfiguration() throws Exception{
        
        try {
            
            String configFilePath = tomcatConfPath + File.separator + "ism-configuration.properties";
            
            //First take a backup of config file
            System.out.println("Backing up " + "ism-configuration.properties" + " to " + configBackupDir);
            FileUtils.copyFileToDirectory(new File(configFilePath), configBackupDir);
            
            System.out.println("Updating ism-configuration.properties");
            
            ismConfig = new  IsmConfiguration(configFilePath);
            
            ConfigurationNode root = configChangesTemplate.getRootNode();
            
            ConfigurationNode ismConfigNode = root.getChildren("ism-configuration").get(0);
            
            List<ConfigurationNode> nodes = ismConfigNode.getChildren();
            
            for(ConfigurationNode node:nodes){
                
                if(node.getName().equals("set-property")){
                   ismConfig.setProperty((String)node.getAttribute(0).getValue(), (String)node.getValue()) ; 
                   
                }else if(node.getName().equals("delete-property")){
                   ismConfig.deleteProperty((String)node.getAttribute(0).getValue()) ;
                   
                }else{
                    System.err.println("UpdateIsmConfiguration: Invalid configuration " + node.getName());
                }
            }
            
        }catch(Exception e){
            e.printStackTrace();
            throw new Exception("Could not update ism-configuration.properties\n" + e.getMessage());

        }
        
    }
    
    public static void updateIdmUserAppLoggingXML() throws Exception{
        try {
            
            String configFilePath = tomcatConfPath + File.separator + "idmuserapp_logging.xml";
            
            //First take a backup of config file
            System.out.println("Backing up " + "idmuserapp_logging.xml" + " to " + configBackupDir);
            FileUtils.copyFileToDirectory(new File(configFilePath), configBackupDir);
            
            System.out.println("Updating idmuserapp_logging.xml");
            
            idmUserAppLoggingXML = new IdmUserAppLogging(configFilePath);
            
            ConfigurationNode root = configChangesTemplate.getRootNode();
            
            ConfigurationNode idmuserapp_loggingNode = root.getChildren("idmuserapp_logging").get(0);
            
            List<ConfigurationNode> nodes = idmuserapp_loggingNode.getChildren();
            
            for(ConfigurationNode node:nodes){
                
                //JOptionPane.showMessageDialog(null, (String)node.getAttribute(0).getValue() + " " + (String)node.getValue());
                
                if(node.getName().equals("add-appender-param")){
                                     
                    idmUserAppLoggingXML.addAppenderParameter(node.getAttributes("appender-name").get(0).getValue().toString(), node.getChild(0));
                   
                }else if(node.getName().equals("delete-appender-param")){
                    idmUserAppLoggingXML.deleteAppenderParameter(node.getAttributes("appender-name").get(0).getValue().toString(), node.getChild(0));
                   
                }else{
                    System.err.println("UpdateIdmUserAppLoggingXML: Invalid configuration " + node.getName());
                }
            }
            
        }catch(Exception e){
            e.printStackTrace();
            throw new Exception("Could not update idmuserapp_logging.xml\n" + e.getMessage());
        }
    }
    
    public static void updateHibernate_cfgXML() throws Exception{
        try {
            
            String configFilePath = tomcatConfPath + File.separator + "hibernate.cfg.xml";
            
            //First take a backup of config file
            System.out.println("Backing up " + "hibernate.cfg.xml" + " to " + configBackupDir);
            FileUtils.copyFileToDirectory(new File(configFilePath), configBackupDir);
            
            System.out.println("Updating hibernate.cfg.xml");
            
            hibernateCfg = new HibernateCfg(configFilePath);
            
            ConfigurationNode root = configChangesTemplate.getRootNode();
            
            ConfigurationNode idmuserapp_loggingNode = root.getChildren("hibernate_cfg").get(0);
            
            List<ConfigurationNode> nodes = idmuserapp_loggingNode.getChildren();
            
            for(ConfigurationNode node:nodes){
                
                
                if(node.getName().equals("add-class-mapping")){
                                     
                    hibernateCfg.addClassMapping(node.getChild(0).getAttributes("class").get(0).getValue().toString());
                    
                   
                }else if(node.getName().equals("add-resource-mapping")){
                    
                    hibernateCfg.addResorceMapping(node.getChild(0).getAttributes("resource").get(0).getValue().toString());
                    
                   
                }else{
                    System.err.println("updateHibernate_cfgXML: Invalid configuration " + node.getName());
                }
            }
            
        }catch(Exception e){
            e.printStackTrace();
            throw new Exception("Could not update hibernate.cfg.xml\n" + e.getMessage());
        }
    }

}
