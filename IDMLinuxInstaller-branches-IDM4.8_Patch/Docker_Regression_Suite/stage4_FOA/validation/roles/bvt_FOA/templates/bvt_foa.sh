...
path='/opt/novell/dirxml/fanoutagent/lib/'

declare -A defvers
{% for key, value in foa.items() %}
defvers["{{ key }}"]={{ value }}
{% endfor %}

echo "   <h2>FanOut Agent jars versions check</h2>
...
curr=$path"FanoutAgent.jar"
version=$(unzip -p $curr META-INF/MANIFEST.MF | grep -m 1 'Bundle-Version\|Implementation-Version\|Product-Version\|Specification-Version'|awk '{ print $2  }')
printf "\n\n" >> result
echo "Driver             Version" >> result
echo "--------------------------------------" >> result
echo "FanoutAgent        $version" >> result
echo "Fanout-Agent $total $pass $fail $unknown" > bvt_result
