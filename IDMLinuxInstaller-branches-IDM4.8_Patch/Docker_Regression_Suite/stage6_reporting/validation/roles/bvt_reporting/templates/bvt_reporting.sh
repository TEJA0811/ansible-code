...
set -x 
touch {{ Automation_dir }}/reporting.html
touch {{ Automation_dir }}/result

pass=0
fail=0
unknown=0

true>{{ Automation_dir }}/reporting.html
            ...
            <div id="bvt-version">
                <div id="bvt-version-table">
                    <table id="version-table" class="table table-striped table-bordered" cellspacing="0" width="100%" class="tablesorter">
                        ...
                        </thead>">>{{ Automation_dir }}/reporting.html
j=1
...
################# Print results #########################
echo "$total Test cases Total" > {{ Automation_dir }}/result
echo "$pass Test cases Passed" >> {{ Automation_dir }}/result
echo "$unknown Test cases Version not found" >> {{ Automation_dir }}/result
echo "$fail Test cases Failed" >> {{ Automation_dir }}/result
echo "***************************" >> {{ Automation_dir }}/result
echo "Reporting $total $pass $fail $unknown" > {{ Automation_dir }}/bvt_result
