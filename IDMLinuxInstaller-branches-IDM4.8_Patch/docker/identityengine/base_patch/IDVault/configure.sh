...
#			ID_VAULT_TREENAME="$ID_VAULT_TREENAME-`date +'%y%m%d%H%M'`"
		...
		write_and_log "$str1"
		RC=0	
		IPsforEBA=$(ip a | grep inet | grep -Ev "inet6|host lo| docker[0-9]| virbr[0-9]" | cut -d"/" -f1 | cut -d" " -f6)
		for i in $IPsforEBA
		do
			if [ -z $IPswithNCP ]
			then
				IPswithNCP=$i@${ID_VAULT_NCP_PORT}
			else
				IPswithNCP=$IPswithNCP,$i@${ID_VAULT_NCP_PORT}
			fi
		done
		if [ ! -z $IPswithNCPoverride ]
		then
			IPswithNCP=$IPswithNCPoverride
		fi
		if [ -z $IPswithNCP ]
		then
			IPswithNCP=${ID_VAULT_HOST}@${ID_VAULT_NCP_PORT}
		fi
		...
		if [ -z $forebaenable ]
		then
			forebaenable="--configure-eba-now yes"
		else
			forebaenable=$forebaenable
		fi
		if [ ! -z ${KUBERNETES_ORCHESTRATION} ] && [ "${KUBERNETES_ORCHESTRATION}" == "y" ]
		then
			forebaenable="--configure-eba-now yes"
